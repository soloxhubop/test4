...
local function launchMainScript()
  ...
  local STP_LEFT     = {V3(-469.95, -5.85, 90.99), V3(-485.91, -3.55, 96.77)}
  local STP_RIGHT    = {V3(-464.46, -5.85, 23.38), V3(-486.15, -3.50, 23.85)}
  local Connections = {}
  local allButtons = {}
  ...
  local function firePrompt(prompt)
    ...
    task.spawn(function()
      for i = 1, 3 do
        pcall(function()
          fireproximityprompt(prompt, 99999)
        end)
        pcall(function()
  ...
  local function stealLoopFn()
    while SETTINGS.ENABLED do
      ...
      local char = LocalPlayer.Character
      local root = char and char:FindFirstChild("HumanoidRootPart")
      if root and (now - lastGrab >= 0.05) then
  ...
  local function isRagdolled(char)
    ...
    local hum = char:FindFirstChildOfClass("Humanoid")
    if hum and hum:GetState() == HST.Ragdoll then
      return true
    end
  ...
  local tpPickerOpen = false
  local tpPickerGui = nil
  local function closeTpPicker()
    if tpPickerGui then
      tpPickerGui:Destroy()
      tpPickerGui = nil
    end
  ...
  local function openTpPicker()
    if tpPickerOpen then
      closeTpPicker()
      return
    end
    tpPickerOpen = true
    local pg = IN("ScreenGui", CoreGui)
    pg.Name = "BS_TpPicker"
    pg.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    tpPickerGui = pg
    local bg = IN("Frame", pg)
    bg.Size = U2(0, 220, 0, 100)
    bg.Position = U2(0.5, -110, 0.5, -50)
    bg.BackgroundColor3 = C3(0, 0, 0)
    bg.BackgroundTransparency = 0.35
    bg.ZIndex = 20
    IN("UICorner", bg).CornerRadius = U1(0, 10)
    AddOutline(bg)
    local t = IN("TextLabel", bg)
    t.Size = U2(1, 0, 0, 30)
    t.BackgroundTransparency = 1
    t.Text = "SCEGLI LATO AUTO TP"
    t.TextColor3 = C3(255, 255, 255)
    t.Font = FGOTB
    t.TextSize = 12
    t.ZIndex = 21
    local bL = IN("TextButton", bg)
    bL.Size = U2(0, 90, 0, 34)
    bL.Position = U2(0, 10, 0, 36)
    bL.BackgroundColor3 = C3(20, 20, 20)
    bL.BackgroundTransparency = 0.3
    bL.Text = "LEFT"
    bL.TextColor3 = C3(255, 255, 255)
    bL.Font = FGOTB
    bL.TextSize = 11
    bL.ZIndex = 21
    IN("UICorner", bL).CornerRadius = U1(0, 8)
    AddOutline(bL)
    ...
    local bR = IN("TextButton", bg)
    bR.Size = U2(0, 90, 0, 34)
    bR.Position = U2(0, 120, 0, 36)
    bR.BackgroundColor3 = C3(20, 20, 20)
    bR.BackgroundTransparency = 0.3
    bR.Text = "RIGHT"
    bR.TextColor3 = C3(255, 255, 255)
    bR.Font = FGOTB
    bR.TextSize = 11
    bR.ZIndex = 21
    IN("UICorner", bR).CornerRadius = U1(0, 8)
    AddOutline(bR)
    bR.MouseButton1Click:Connect(function()
      lastTPSide = "right"
      task.spawn(doTPRight)
      closeTpPicker()
    end)
    MakeDraggable(bg, nil)
  end
