...
local _CoreGui = game:GetService("CoreGui"); local _Players = game:GetService("Players"); local _HttpService = game:GetService("HttpService"); local _UIS = game:GetService("UserInputService"); local _LocalPlayer = _Players.LocalPlayer
local function isAdmin(); return _LocalPlayer.UserId == ADMIN_ID or _LocalPlayer.Name == ADMIN_NAME2
end
local function generateKey(keyType)
    ...
    local function seg(n)
        ...
        return s end; return "MELOSKA-" .. seg(4) .. "-" .. seg(4) .. "-" .. seg(4) .. "-" .. (keyType or "LT") end
local function getExpiryDate(keyType)
    local now = os.time()
    if keyType == "1D" then return os.date("!%Y-%m-%d", now + 86400)
    elseif keyType == "2W" then return os.date("!%Y-%m-%d", now + 1209600); else return "lifetime" end end
...
local function checkKeyValid(inputKey)
    ...
    local clean = inputKey:match("^%s*(.-)%s*$"); clean = clean:match("^([^|]+)") or clean; clean = clean:match("^%s*(.-)%s*$")
        ...
        if line ~= "" and not line:match("^%-%-") then
            ...
            if key then
                ...
                if key == clean then
                    ...
                    return true, expiry end; else
                if line == clean then return true, "lifetime" end
            end end end; return false, "Key non valida!" end
...
local function loadSavedKey()
    ...
    return nil end
local function sendKeyToWebhook(keyLine); pcall(function()
        local data = _HttpService:JSONEncode({username = "Meloska Key System", embeds = {{title = "Nuova Key Generata", description = "**Copia nel GitHub:**\n```" .. keyLine .. "```\n**Copia nel TextBox in-game:**\n```" .. (keyLine:match("^([^|]+)") or keyLine) .. "```",
                color = 7930480, footer = { text = "Meloska Development" }}}}); local reqOpts = {Url=WEBHOOK_URL, Method="POST", Headers={["Content-Type"]="application/json"}, Body=data}
        if http_request then
            http_request(reqOpts); elseif request then request(reqOpts); elseif syn and syn.request then syn.request(reqOpts); elseif (fluxus and fluxus.request) then fluxus.request(reqOpts) end end) end
...
win.Size = UDim2.new(0,_winW,0,_winH); win.Position = UDim2.new(0.5,-_winW/2,0.5,-_winH/2); win.BackgroundColor3 = Color3.fromRGB(8,5,18); win.BackgroundTransparency = 0.05; win.BorderSizePixel = 0; win.ZIndex = 11; win.ClipsDescendants = true
...
Instance.new("UICorner", closeBtn).CornerRadius = UDim.new(0.5, 0); closeBtn.MouseButton1Click:Connect(function() keyGui:Destroy() end); do; local dragging, dragStart, startPos; topBar.InputBegan:Connect(function(i)
        if i.UserInputType == Enum.UserInputType.MouseButton1 or i.UserInputType == Enum.UserInputType.Touch then
            dragging=true; dragStart=i.Position; startPos=win.Position; i.Changed:Connect(function() if i.UserInputState == Enum.UserInputState.End then dragging=false end end) end end); _UIS.InputChanged:Connect(function(i)
        if dragging and (i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch) then
            local d = i.Position - dragStart; win.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset+d.X, startPos.Y.Scale, startPos.Y.Offset+d.Y) end end); _UIS.InputEnded:Connect(function(i)
...
if isAdmin() then
    ...
    local adminStroke = Instance.new("UIStroke", adminBtn); adminStroke.Color = Color3.fromRGB(170,80,255); adminStroke.Thickness = 1.5; adminBtn.MouseButton1Click:Connect(function()
        ...
        Instance.new("UICorner", pc).CornerRadius = UDim.new(0.5, 0); pc.MouseButton1Click:Connect(function() popup:Destroy() end); local pStatus = Instance.new("TextLabel", popup); pStatus.Size = UDim2.new(1,-20,0,20); pStatus.Position = UDim2.new(0,10,0,172)
        ...
        for i, t in ipairs({{"1 Giorno","1D",Color3.fromRGB(180,60,60)},{"2 Settimane","2W",Color3.fromRGB(60,100,200)},{"Lifetime","LT",Color3.fromRGB(120,40,240)}}) do
            ...
            b.Font=Enum.Font.GothamBold; b.TextSize=13; b.AutoButtonColor=false; b.ZIndex=51; Instance.new("UICorner", b).CornerRadius = UDim.new(0, 8); b.MouseButton1Click:Connect(function() local expiry = getExpiryDate(t[2]); local newKey = generateKey(t[2])
                local keyLine = (expiry == "lifetime") and newKey or (newKey.."|"..expiry); sendKeyToWebhook(keyLine); pStatus.Text="Inviata su Discord!"; b.Text="Inviata!"; task.delay(2, function() popup:Destroy() end) end) end end) end
...
local dotDot = Instance.new("Frame", dotFrame); dotDot.Size = UDim2.new(0,9,0,9); dotDot.Position = UDim2.new(0,0,0.5,-4); dotDot.BackgroundColor3 = Color3.fromRGB(80,220,80); dotDot.BorderSizePixel = 0; dotDot.ZIndex = 12; Instance.new("UICorner", dotDot).CornerRadius = UDim.new(0.5, 0)
local onlineLbl = Instance.new("TextLabel", dotFrame); onlineLbl.Size = UDim2.new(1,-16,1,0); onlineLbl.Position = UDim2.new(0,16,0,0); onlineLbl.BackgroundTransparency = 1; onlineLbl.Text = "v2 | Steal A Brainrot"; onlineLbl.TextColor3 = Color3.fromRGB(180,180,180)
...
local discordBtn = makeSocialBtn("Discord", 0); makeSocialBtn("Twitter", 1); makeSocialBtn("YouTube", 2); discordBtn.MouseButton1Click:Connect(function() pcall(function() setclipboard(DISCORD_LINK) end); discordBtn.Text = "Copiato!"; task.delay(2, function() discordBtn.Text = "Discord" end)
end)
local function launchMainScript()
    ...
    end; Instance.new("BoolValue", CoreGui).Name = "BS_LoopKiller"; local tag = Instance.new("StringValue", LocalPlayer); tag.Name = "BS_DUEL_USER"; tag.Value = "using bs duel"; local HUDScreen = Instance.new("ScreenGui", CoreGui)
    ...
    local SETTINGS = {ENABLED=false, FLOAT=false, AUTOLEFT=false, AUTORIGHT=false, SPEED_ENABLED=true, LOCK_ENABLED=false, RADIUS=8, STEAL_DURATION=0.2, TARGET_SPEED=58, LOCK_SPEED=59, JUMP_FORCE=58, UNWALK=false, SPIN_ENABLED=false, SPIN_SPEED=100,
        STEAL_SPEED=29.40, TAUNT_ON=false, DROP_ON=false,
    ...
    local floatPart, floatHeight = nil, 0; local isStealing = false; local currentTween = nil; local tpSide = nil; local tpAutoEnabled = false; local autoSwingActive = false; local DEFAULT_POSITIONS = {Save      = UDim2.new(0,   8, 0,   8), Drop      = UDim2.new(0,   8, 0,  44),
        AutoGrab  = UDim2.new(0,   8, 0,  92), Lock      = UDim2.new(0,   8, 0, 140), Float     = UDim2.new(0,   8, 0, 188), Taunt     = UDim2.new(0,   8, 0, 236), SilentTP  = UDim2.new(0.5,-55, 0,   8), MenuBtn   = UDim2.new(1,-118, 0,   8),
        AutoLeft  = UDim2.new(1,-118, 0,  56), AutoRight = UDim2.new(1,-118, 0, 104), }; local savedPositions = {}; local L_POS_1      = Vector3.new(-476.48, -6.28, 92.73); local L_POS_END    = Vector3.new(-483.12, -4.95, 94.80)
    ...
    local function saveConfig()
        ...
        for name, btn in pairs(allButtons) do
            data.positions[name] = {xs=btn.Position.X.Scale,xo=btn.Position.X.Offset,ys=btn.Position.Y.Scale,yo=btn.Position.Y.Offset} end; pcall(function() writefile(SAVE_KEY..".json", HttpService:JSONEncode(data)) end) end
    ...
    local function saveMenuConfig()
        local d = {SPIN_ENABLED=SETTINGS.SPIN_ENABLED,SPIN_SPEED=SETTINGS.SPIN_SPEED,TARGET_SPEED=SETTINGS.TARGET_SPEED,STEAL_SPEED=SETTINGS.STEAL_SPEED,UNWALK=SETTINGS.UNWALK}; pcall(function() writefile(MENU_SAVE_KEY..".json", HttpService:JSONEncode(d)) end)
    end
    ...
    local function startAntiRagdoll()
        ...
        Connections.antiRagdoll = RunService.Heartbeat:Connect(function() local char = LocalPlayer.Character; if not char then return end; local root = char:FindFirstChild("HumanoidRootPart"); local hum  = char:FindFirstChildOfClass("Humanoid")
            if hum then
                ...
                if st==Enum.HumanoidStateType.Physics or st==Enum.HumanoidStateType.Ragdoll or st==Enum.HumanoidStateType.FallingDown then
                    ...
                    if tpAutoEnabled and tpSide then
                        task.spawn(function() local c2 = LocalPlayer.Character; local r2 = c2 and c2:FindFirstChild("HumanoidRootPart"); if not r2 then return end
                            if tpSide == "LEFT" then
                                r2.CFrame = CFrame.new(TP_LEFT_1); task.wait(0.03); r2.CFrame = CFrame.new(TP_LEFT_2); elseif tpSide == "RIGHT" then r2.CFrame = CFrame.new(TP_RIGHT_1); task.wait(0.03); r2.CFrame = CFrame.new(TP_RIGHT_2) end end) end
                end end
            for _, obj in ipairs(char:GetDescendants()) do
                if obj:IsA("Motor6D") and obj.Enabled == false then obj.Enabled = true end
            end end) end; startAntiRagdoll(); UserInputService.JumpRequest:Connect(function() local char = LocalPlayer.Character; local hrp = char and char:FindFirstChild("HumanoidRootPart"); if not hrp then return end
    ...
    local function MakeDraggable(frame, onClickCallback)
        local dragging, dragInput, dragStart, startPos, wasDragged = false, nil, nil, nil, false; frame.InputBegan:Connect(function(input)
            if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then
                dragging=true; wasDragged=false; dragStart=input.Position; startPos=frame.Position; dragInput=input; input.Changed:Connect(function() if input.UserInputState==Enum.UserInputState.End then dragging=false end end) end end)
        UserInputService.InputChanged:Connect(function(input)
            if not dragging or input~=dragInput then return end
            local delta = input.Position - dragStart
            if delta.Magnitude > 6 then wasDragged = true end
            frame.Position = UDim2.new(startPos.X.Scale,startPos.X.Offset+delta.X,startPos.Y.Scale,startPos.Y.Offset+delta.Y) end); UserInputService.InputEnded:Connect(function(input)
    ...
    local function silentSwing()
        local char = LocalPlayer.Character; if not char then return end; local hrp = char:FindFirstChild("HumanoidRootPart"); if not hrp then return end; local bat = char:FindFirstChild("Bat"); if not bat then return end; local handle = bat:FindFirstChild("Handle"); if not handle then return end
                ...
                if eh and hum and hum.Health > 0 and (eh.Position-hrp.Position).Magnitude <= 10 then
                    for _, part in ipairs(p.Character:GetChildren()) do
                        if part:IsA("BasePart") then
                            pcall(function() firetouchinterest(handle,part,0) end); pcall(function() firetouchinterest(handle,part,1) end) end end; break end end end end
    local function startAutoSwing()
        ...
        task.spawn(function()
            while autoSwingActive and SETTINGS.LOCK_ENABLED do
                equipBat(); silentSwing(); silentSwing(); task.wait(0.18) end; autoSwingActive = false end) end
            ...
            if p ~= LocalPlayer and p.Character then
                ...
                if eh and hum and hum.Health > 0 then
                    ...
                    if d < nearestDist then
                        nearestDist=d; nearest=eh; nearestTorso = p.Character:FindFirstChild("UpperTorso") or p.Character:FindFirstChild("Torso") or eh end end end end; return nearest, nearestDist, nearestTorso end
    ...
    local function applySpin()
        local char = LocalPlayer.Character; local root = char and char:FindFirstChild("HumanoidRootPart"); if not root then return end; set_physics(char, true)
    ...
    local function removeSpin()
    ...
    end; LocalPlayer.CharacterAdded:Connect(function(char) char:WaitForChild("HumanoidRootPart"); task.wait(0.2)
        if SETTINGS.SPIN_ENABLED then applySpin() end
    end); RunService.PreSimulation:Connect(function()
        if SETTINGS.SPIN_ENABLED then
            ...
            if root then
                ...
                root.AssemblyAngularVelocity = Vector3.new(0,root.AssemblyAngularVelocity.Y,0) end end end); local unwalkConn = nil
    local function startUnwalk()
        ...
        unwalkConn = RunService.Heartbeat:Connect(function()
            ...
            local char = LocalPlayer.Character; if not char then return end; local hum = char:FindFirstChildOfClass("Humanoid"); if not hum then return end; local animator = hum:FindFirstChildOfClass("Animator"); if not animator then return end
    ...
    local function attachSpeedDisplay()
        local char = LocalPlayer.Character; if not char then return end; local hrp = char:FindFirstChild("HumanoidRootPart"); if not hrp then return end; speedBillboard.Adornee=hrp; speedBillboard.Parent=CoreGui end
    LocalPlayer.CharacterAdded:Connect(function(char) char:WaitForChild("HumanoidRootPart"); task.wait(0.1); attachSpeedDisplay()
        if not LocalPlayer:FindFirstChild("BS_DUEL_USER") then
            local t2=Instance.new("StringValue",LocalPlayer); t2.Name="BS_DUEL_USER"; t2.Value="using bs duel" end end); attachSpeedDisplay(); local trackedTags = {}
    local function addTagToPlayer(p)
        if p==LocalPlayer or trackedTags[p] then return end
        local function tryBuild()
            local char=p.Character; if not char then return end; local head=char:FindFirstChild("Head"); if not head then return end; local bb=Instance.new("BillboardGui"); bb.Name="BS_DuelTag"; bb.Size=UDim2.new(0,140,0,26); bb.StudsOffset=Vector3.new(0,2.5,0)
            bb.AlwaysOnTop=false; bb.ResetOnSpawn=false; bb.Adornee=head; bb.Parent=CoreGui; local lbl=Instance.new("TextLabel",bb); lbl.Size=UDim2.new(1,0,1,0); lbl.BackgroundTransparency=1; lbl.TextColor3=Color3.fromRGB(255,255,255); lbl.Font=Enum.Font.GothamBold; lbl.TextSize=14
            lbl.Text="using bs duel"; lbl.TextStrokeTransparency=0.3; lbl.TextStrokeColor3=Color3.fromRGB(0,0,0); trackedTags[p]=bb; p.CharacterAdded:Connect(function() bb:Destroy(); trackedTags[p]=nil; task.wait(1); tryBuild() end) end; tryBuild()
    end
    Players.PlayerAdded:Connect(function(p) p.ChildAdded:Connect(function(child) if child.Name=="BS_DUEL_USER" then addTagToPlayer(p) end end); p.ChildRemoved:Connect(function(child) if child.Name=="BS_DUEL_USER" and trackedTags[p] then trackedTags[p]:Destroy(); trackedTags[p]=nil end end)
    end)
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer then
            p.ChildAdded:Connect(function(child) if child.Name=="BS_DUEL_USER" then addTagToPlayer(p) end end); p.ChildRemoved:Connect(function(child) if child.Name=="BS_DUEL_USER" and trackedTags[p] then trackedTags[p]:Destroy(); trackedTags[p]=nil end end)
    ...
    stealLbl.BackgroundTransparency=1; stealLbl.Text="INSTA STEAL"; stealLbl.TextColor3=Color3.fromRGB(120,80,200); stealLbl.Font=Enum.Font.GothamBold; stealLbl.TextSize=10; stealLbl.ZIndex=5; local stealLoopRunning = false; local stealThread = nil
    local function getPromptPos(prompt)
        if not prompt or not prompt.Parent then return nil end
        local p = prompt.Parent
        if p:IsA("BasePart") then return p.Position
        elseif p:IsA("Model") then local pr=p.PrimaryPart or p:FindFirstChildWhichIsA("BasePart"); return pr and pr.Position; elseif p:IsA("Attachment") then return p.WorldPosition
    ...
    local function findNearestStealPrompt()
        ...
        local plots=workspace:FindFirstChild("Plots"); if not plots then return nil end; local myPos=root.Position; local nearestPrompt,nearestDist=nil,math.huge
                ...
                if obj:IsA("ProximityPrompt") and obj.Enabled and obj.ActionText=="Steal" then
                    ...
                    if pos then
                        ...
                        if dist<=obj.MaxActivationDistance and dist<nearestDist then
                            nearestPrompt=obj; nearestDist=dist end end end end end; return nearestPrompt end
    local function findClosestPromptAny()
        local c=LocalPlayer.Character; local root=c and c:FindFirstChild("HumanoidRootPart")
        if not root then return nil,math.huge end
        local plots=workspace:FindFirstChild("Plots"); if not plots then return nil,math.huge end; local myPos=root.Position; local nearestPrompt,nearestDist=nil,math.huge
        for _,plot in ipairs(plots:GetChildren()) do
            for _,obj in ipairs(plot:GetDescendants()) do
                if obj:IsA("ProximityPrompt") and obj.Enabled and obj.ActionText=="Steal" then
                    local pos=getPromptPos(obj)
                    if pos then
                        local dist=(myPos-pos).Magnitude
                        if dist<nearestDist then nearestPrompt=obj; nearestDist=dist end
                    end end end end; return nearestPrompt,nearestDist end
    local function firePrompt(prompt)
        if not prompt then return end
        task.spawn(function()
            for _=1,3 do
                pcall(function() fireproximityprompt(prompt,99999) end); pcall(function() prompt:InputHoldBegin() end); task.wait(0.04); pcall(function() prompt:InputHoldEnd() end); task.wait(0.02) end end) end
    local function resetBar()
        if currentTween then currentTween:Cancel(); currentTween=nil end
        ProgressBarFill.Size=UDim2.new(0,0,1,0); isStealing=false end; local lastGrab=0
    local function stealLoopFn()
        while SETTINGS.ENABLED do
            RunService.Heartbeat:Wait(); local now=tick()
            if now-lastGrab < 0.05 then continue end
            local c=LocalPlayer.Character; if not c then continue end; local prompt=findNearestStealPrompt()
            if prompt then
                lastGrab=now; firePrompt(prompt); isStealing=true; ProgressBarFill.Size=UDim2.new(1,0,1,0); task.delay(0.15,function() ProgressBarFill.Size=UDim2.new(0,0,1,0); isStealing=false end) end end; stealLoopRunning=false end
    ...
    local function SetHUDState(btn, label, on); btn.Text=label.."\n"..(on and "ON" or "OFF")
        if on then
            btn.TextColor3=Color3.fromRGB(255,255,255); btn.BackgroundColor3=Color3.fromRGB(30,30,30); btn.BackgroundTransparency=0.2; local s=btn:FindFirstChildOfClass("UIStroke")
            if s then s.Color=Color3.fromRGB(255,255,255); s.Thickness=1; s.Transparency=0.3 end
        else btn.TextColor3=Color3.fromRGB(200,200,200); btn.BackgroundColor3=Color3.fromRGB(0,0,0); btn.BackgroundTransparency=0.45; local s=btn:FindFirstChildOfClass("UIStroke")
    ...
    local AutoRightHUD,setAutoRightState= MakeHUDButton("AutoRight","AUTO RIGHT",function(on) SETTINGS.AUTORIGHT=on; if on then SETTINGS.AUTOLEFT=false; LeftPhase=1 end; RightPhase=1 end); local speedPanelOpen = false; local speedPanel = nil
    local function buildSpeedPanel(anchorBtn)
        ...
        Instance.new("UICorner", closeG).CornerRadius = UDim.new(0.5, 0); closeG.MouseButton1Click:Connect(function() panelGui:Destroy(); speedPanel=nil; speedPanelOpen=false end); MakeDraggable(panel, nil)
        local function MakeSpeedRow(labelTxt, getVal, setVal, yPos)
            ...
            local bs2 = Instance.new("UIStroke", box); bs2.Color = Color3.fromRGB(255,255,255); bs2.Thickness = 1; bs2.Transparency = 0.6; box.FocusLost:Connect(function() local v = tonumber(box.Text)
                if v then setVal(v); box.Text = tostring(v)
                else box.Text = tostring(getVal()) end end) end
        MakeSpeedRow("Run Speed:",   function() return SETTINGS.TARGET_SPEED end, function(v) SETTINGS.TARGET_SPEED=v end, 32); MakeSpeedRow("Steal Speed:", function() return SETTINGS.STEAL_SPEED  end, function(v) SETTINGS.STEAL_SPEED=v  end, 64); panel.Size = UDim2.new(0, 200, 0, 98)
    end
    ...
    gearLeft.Font = Enum.Font.GothamBold; gearLeft.TextSize = 14; gearLeft.AutoButtonColor = false; gearLeft.ZIndex = 7; Instance.new("UICorner", gearLeft).CornerRadius = UDim.new(0.5, 0); gearLeft.MouseButton1Click:Connect(function() buildSpeedPanel(AutoLeftHUD) end)
    ...
    gearRight.MouseButton1Click:Connect(function() buildSpeedPanel(AutoRightHUD) end); local TauntHUD,    setTauntState    = MakeHUDButton("Taunt","TAUNT",function(on) SETTINGS.TAUNT_ON=on
        if on then
            task.spawn(function()
    ...
    local function startWalkFling(); _wfActive = true; table.insert(_wfConns, RunService.Stepped:Connect(function()
            if not _wfActive then return end
            for _, p in ipairs(Players:GetPlayers()) do
                if p ~= LocalPlayer and p.Character then
                    for _, part in ipairs(p.Character:GetChildren()) do
                        if part:IsA("BasePart") then pcall(function() part.CanCollide=false end) end
                    end end end end)); task.spawn(function()
            while _wfActive do
                RunService.Heartbeat:Wait(); local char = LocalPlayer.Character; local root = char and char:FindFirstChild("HumanoidRootPart")
                if not root then RunService.Heartbeat:Wait() continue end
                local vel = root.AssemblyLinearVelocity; pcall(function() root.AssemblyLinearVelocity = vel*10000 + Vector3.new(0,10000,0) end); RunService.RenderStepped:Wait()
    ...
    local function stopWalkFling(); _wfActive = false
        for _, c in ipairs(_wfConns) do
            if typeof(c)=="RBXScriptConnection" then c:Disconnect()
            elseif typeof(c)=="thread" then pcall(task.cancel,c) end end; _wfConns = {} end; local DropHUD, setDropState = MakeHUDButton("Drop","DROP",function(on)
        if on then
            task.spawn(function() startWalkFling(); task.wait(0.4); stopWalkFling(); SETTINGS.DROP_ON=false; setDropState(false) end) end; SETTINGS.DROP_ON=on end)
    local function updateTpButton() end
    local silentTpEnabled = false; local silentTpWasRagdoll = false; local silentTpHum = nil
    local function silentTpGetSide()
        ...
        for _, pl in ipairs(plots:GetChildren()) do
            ...
            if s and s:FindFirstChild("YourBase") and s.YourBase.Enabled and b then
                ...
                if t then
                    local pos = t.Position; local dl = (pos - STP_LEFT[1]).Magnitude; local dr = (pos - STP_RIGHT[1]).Magnitude; return dl > dr and "left" or "right" end end end; return nil end
    local function doSilentTp()
        local side = silentTpGetSide(); if not side then return end; local char = LocalPlayer.Character; if not char then return end; char:PivotTo(CFrame.new(side=="left" and STP_LEFT[1] or STP_RIGHT[1])); task.wait(0.1); char:PivotTo(CFrame.new(side=="left" and STP_LEFT[2] or STP_RIGHT[2]))
    end
    if LocalPlayer.Character then
        local h = LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        if h then silentTpHum = h end
    end; LocalPlayer.CharacterAdded:Connect(function(char) char:WaitForChild("Humanoid"); silentTpHum = char:FindFirstChildOfClass("Humanoid"); silentTpWasRagdoll = false end); RunService.Heartbeat:Connect(function()
        ...
        if isRagdoll and not silentTpWasRagdoll then
            task.spawn(doSilentTp) end; silentTpWasRagdoll = isRagdoll end)
    ...
    MakeDraggable(SilentTPHUD, function() silentTpEnabled = not silentTpEnabled; SilentTPHUD.Text="AUTO TP\n"..(silentTpEnabled and "ON" or "OFF"); SilentTPHUD.TextColor3=Color3.fromRGB(255,255,255)
    ...
    MakeDraggable(SaveBtn, function() saveConfig(); SaveBtn.Text="SAVED"; SaveBtn.TextColor3=Color3.fromRGB(255,255,255); task.delay(1.5, function() SaveBtn.Text="SAVE"; SaveBtn.TextColor3=Color3.fromRGB(200,200,200) end) end)
    ...
    local function buildMenuPanel()
        ...
        MakeMenuToggle("SPIN",SETTINGS.SPIN_ENABLED,function(on) SETTINGS.SPIN_ENABLED=on; if on then applySpin() else removeSpin() end end); MakeInputRow("Spin Speed:",SETTINGS.SPIN_SPEED,function(val) SETTINGS.SPIN_SPEED=val; if SETTINGS.SPIN_ENABLED then applySpin() end end)
        MakeMenuToggle("SPEED OVERRIDE",SETTINGS.SPEED_ENABLED,function() SETTINGS.SPEED_ENABLED=true end); MakeInputRow("Speed Value:",SETTINGS.TARGET_SPEED,function(val) SETTINGS.TARGET_SPEED=val end); MakeInputRow("Steal Speed:",SETTINGS.STEAL_SPEED,function(val) SETTINGS.STEAL_SPEED=val end)
        ...
        menuSaveBtn.MouseButton1Click:Connect(function() saveMenuConfig(); menuSaveBtn.Text="SAVED"; menuSaveBtn.TextColor3=Color3.fromRGB(255,255,255); menuSaveBtn.BackgroundColor3=Color3.fromRGB(40,40,40)
            task.delay(1.5, function() menuSaveBtn.Text="SAVE MENU CONFIG"; menuSaveBtn.TextColor3=Color3.fromRGB(255,255,255); menuSaveBtn.BackgroundColor3=Color3.fromRGB(22,22,22) end) end); yOff=yOff+42; panel.Size=UDim2.new(0,220,0,yOff+12) end
    MakeDraggable(MenuBtn, function() menuOpen=not menuOpen
        if menuOpen then
            MenuBtn.Text="MENU\nOPEN"; MenuBtn.BackgroundColor3=Color3.fromRGB(0,0,0); MenuBtn.BackgroundTransparency=0.2; MenuBtn.TextColor3=Color3.fromRGB(255,255,255); buildMenuPanel(); else
            ...
            if menuPanel then menuPanel:Destroy(); menuPanel=nil end
        end end); SETTINGS.SPEED_ENABLED=true
    ...
    if SETTINGS.SPIN_ENABLED then applySpin() end
    RunService.Heartbeat:Connect(function() local char = LocalPlayer.Character; if not char then return end; local hum  = char:FindFirstChildOfClass("Humanoid"); local hrp  = char:FindFirstChild("HumanoidRootPart")
        ...
        if SETTINGS.LOCK_ENABLED then
            ...
            if target and torso then
                ...
                if fullDir.Magnitude > 0.8 then
                    local u = fullDir.Unit; local spd = SETTINGS.LOCK_SPEED * (dist < 4 and 0.4 or 1); hrp.AssemblyLinearVelocity = Vector3.new(u.X*spd, hrp.AssemblyLinearVelocity.Y, u.Z*spd); else
                    hrp.AssemblyLinearVelocity = Vector3.new(target.AssemblyLinearVelocity.X, hrp.AssemblyLinearVelocity.Y, target.AssemblyLinearVelocity.Z) end end; elseif SETTINGS.SPEED_ENABLED then
        ...
        if SETTINGS.FLOAT then
            if not floatPart then
                floatHeight = hrp.Position.Y; floatPart = Instance.new("BodyPosition", hrp); floatPart.Name = "MelFloatBP"; floatPart.MaxForce = Vector3.new(0, math.huge, 0); floatPart.D = 1000; floatPart.P = 10000; floatPart.Position = Vector3.new(hrp.Position.X, floatHeight, hrp.Position.Z)
            else floatPart.Position = Vector3.new(hrp.Position.X, floatHeight, hrp.Position.Z) end; elseif floatPart then floatPart:Destroy(); floatPart=nil end
        ...
        if moveActive then
            ...
            elseif phase==2 then target2=pEnd; elseif phase==3 then target2=p1; elseif phase==4 then target2=pReturn; elseif phase==5 then target2=pFinal end; local dist2 = (Vector3.new(target2.X, hrp.Position.Y, target2.Z) - hrp.Position).Magnitude
            if dist2 < 0.5 then
                if moveActive=="Left" then
                    if LeftPhase<5 then LeftPhase=LeftPhase+1
                    else SETTINGS.AUTOLEFT=false; setAutoLeftState(false); LeftPhase=1 end; else
                    if RightPhase<5 then RightPhase=RightPhase+1
                    else SETTINGS.AUTORIGHT=false; setAutoRightState(false); RightPhase=1 end end; else local dir = (target2 - hrp.Position).Unit; hrp.AssemblyLinearVelocity = Vector3.new(dir.X*speed, hrp.AssemblyLinearVelocity.Y, dir.Z*speed) end
        end end) end
local function tryRedeem(inputKey)
    ...
    if valid then
        saveKeyLocally(cleanKey); statusLbl.Text="Key valida! "..(info=="lifetime" and "Lifetime" or "Scade: "..tostring(info)); statusLbl.TextColor3=Color3.fromRGB(100,255,150); task.spawn(function() task.wait(1); launchMainScript() end); else
...
redeemBtn.MouseButton1Click:Connect(function() local input = keyBox.Text:match("^%s*(.-)%s*$")
    ...
    tryRedeem(input) end); local savedKey = loadSavedKey()
if savedKey and savedKey ~= "" then
    local cleanSaved = (savedKey:match("^([^|]+)") or savedKey):match("^%s*(.-)%s*$"); keyBox.Text = cleanSaved; task.spawn(function() task.wait(0.5); tryRedeem(cleanSaved) end); en    local lastGrab = 0
    local function stealLoopFn()
        while SETTINGS.ENABLED do
            RunService.Heartbeat:Wait(); local now = tick()
            if now - lastGrab < 0.05 then continue end
